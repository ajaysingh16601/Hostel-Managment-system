...
function Attendance() {
  ...
  const getAttendance = async () => {
    ...
    const response = await apiRequest("attendance/get", "POST", {
      student: student._id,
    });
    ...
    if (data.success) {
      let daysOffCount = 0;
      let thisWeekData = data.attendance
        .filter((day) => {
          if (day.status === "absent") daysOffCount++;
          return new Date(day.date) >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        })
        .map((day) => ({
